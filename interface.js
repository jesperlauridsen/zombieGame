function inventoryReload(inventory, schematics, hero, gameVariables) {
	...
	if (
		...
		document.getElementById('inventoryBackpack').getElementsByTagName('h4')[0].innerHTML === 'Inventory'
	) {
		document.getElementById('inventoryBackpack').innerHTML =
			"<div class='exitInventory' onclick='closeInventory()'>X</div><h4>Inventory</h4><break>";
		for (i = 0; i < Object.keys(inventory).length; i++) {
			...
			document.getElementById('inventoryBackpack').innerHTML +=
				"<span class='inventoryNameDisplay " +
				inventoryColor +
				"'> " +
				inventory[Object.keys(inventory)[i]].name +
				": </span> <span class='inventoryCountDisplay " +
				inventoryColor +
				"'>" +
				inventory[Object.keys(inventory)[i]].amount +
				'</span><break>';
		}
		gameVariables.pickUp = 0;
	} else if (
		...
		document.getElementById('inventoryBackpack').getElementsByTagName('h4')[0].innerHTML === 'Schematics'
	) {
...
function obtainedSchematicShow(schematic, schematics, inventory, heroObject) {
	...
	for (y = 0; y < Object.keys(schematics).length; y++) {
		if (schematic === schematics[Object.keys(schematics)[y]].constructor) {
			...
			for (h = 0; h < schematics[Object.keys(schematics)[y]].materials.length; h++) {
				materials +=
					"<span id='" +
					schematics[Object.keys(schematics)[y]].materials[h].constructor +
					"All'>" +
					schematics[Object.keys(schematics)[y]].materials[h].name +
					': ' +
					"</span><span id='" +
					schematics[Object.keys(schematics)[y]].materials[h].constructor +
					"Process'><span id='" +
					schematics[Object.keys(schematics)[y]].materials[h].constructor +
					"Collected'>" +
					inventory[schematics[Object.keys(schematics)[y]].materials[h].constructor].amount +
					'</span>' +
					'/' +
					"<span id='" +
					schematics[Object.keys(schematics)[y]].materials[h].constructor +
					"Needed'>" +
					schematics[Object.keys(schematics)[y]].materials[h].amount +
					'</span></span><br />';
			}
		}
	}
	document.getElementById('inventoryBackpack').innerHTML =
		"<div id='inventoryExit' class='exitInventory' onclick='closeInventory()'>X</div><h4>" + headLine + '</h4><break>';
	...
	for (u = 0; u < schematics[Object.keys(schematics)[craftNumber]].materials.length; u++) {
		document.getElementById(
			schematics[Object.keys(schematics)[craftNumber]].materials[u].constructor + 'All'
		).className = 'inventoryNameDisplay';
		document.getElementById(
			schematics[Object.keys(schematics)[craftNumber]].materials[u].constructor + 'Process'
		).className = 'inventoryCountDisplay';
		if (
			inventory[schematics[Object.keys(schematics)[craftNumber]].materials[u].constructor].amount >=
			schematics[Object.keys(schematics)[craftNumber]].materials[u].amount
		) {
			document.getElementById(
				schematics[Object.keys(schematics)[craftNumber]].materials[u].constructor + 'Process'
			).style.color = 'green';
	...
	document.getElementById('returnToSchematics').onclick = function () {
		schematicsDisplay(inventory, schematics, heroObject);
	};
	if (craftable === true) {
		document.getElementById('craftSchematic').onclick = function () {
			craftSpecificSchematic(inventory, schematic, heroObject, schematics);
		};
...
function schematicsDisplay(inventory, schematics, hero) {
	...
	document.getElementById('inventoryBackpack').innerHTML =
		"<div id='inventoryExit' class='exitInventory' onclick='closeInventory()'>X</div><h4>Schematics</h4><break>";
	...
	for (u = 0; u < Object.keys(schematics).length; u++) {
		if (schematics[Object.keys(schematics)[u]].obtained === 0) {
			...
			document.getElementById(schematicName).innerHTML = schematics[Object.keys(schematics)[u]].name;
			document.getElementById(schematicName).onclick = function () {
				obtainedSchematicShow(this.id, schematics, inventory, hero);
			};
		}
	}
}
...
function inventoryDisplay(inventory) {
	document.getElementById('inventoryBackpack').innerHTML =
		"<div id='inventryExit' class='exitInventory' onclick='closeInventory()'>X</div><h4>Inventory</h4><break>";
	for (i = 0; i < Object.keys(inventory).length; i++) {
		if (inventory[Object.keys(inventory)[i]].amount === 0) {
			inventoryColor = 'inventoryColorNotActive';
		} else {
	...
	try {
		if (monster.state != 'idle' || monster.health < monster.fullHealth) {
			...
			if (monster.health > (monster.fullHealth / 100) * 80) {
				...
				monster.health <= (monster.fullHealth / 100) * 80 &&
				monster.health > (monster.fullHealth / 100) * 60
			) {
				ctx.fillStyle = 'rgba(255,183,5,1)';
				ctx.fillRect(monster.x - 15, monster.y - 25, 25 * (monster.health / monster.fullHealth), 3);
			} else if (
				monster.health <= (monster.fullHealth / 100) * 60 &&
				monster.health > (monster.fullHealth / 100) * 40
			) {
				ctx.fillStyle = 'rgba(255,135,5,1)';
				ctx.fillRect(monster.x - 15, monster.y - 25, 25 * (monster.health / monster.fullHealth), 3);
			} else if (
				monster.health <= (monster.fullHealth / 100) * 40 &&
				monster.health > (monster.fullHealth / 100) * 20
			) {
...
function recountMedkit(hero) {
	...
	document.getElementById('utilitiesMedkit').innerHTML =
		'6 <br />' + "<span class='smallTextForUtilities'>" + hero.medkit + ' Medkit</span>';
}
...
function GuidePlayerToObjective(missionArray, hero, image, gameVariables) {
	...
	if (
		(missionArray[hero.currentMission][0].type === 'find' &&
			hero.trackerCountdown + 15000 < gameVariables.timeControler.getTime()) ||
		(missionArray[hero.currentMission][0].type === 'interact' &&
			hero.trackerCountdown + 15000 < gameVariables.timeControler.getTime())
	) {
...
function showArrowToMission(missionArray, image, hero, timeControler) {
	if (
		(hero.trackerActivatedTime + 3500 > timeControler.getTime() &&
			missionArray[hero.currentMission][0].type === 'find') ||
		(missionArray[hero.currentMission][0].type === 'interact' &&
			hero.trackerActivatedTime + 3500 > timeControler.getTime())
	) {
		...
		var angle =
			(Math.atan2(missionArray[hero.currentMission][0].y - 300, missionArray[hero.currentMission][0].x - 400) * 180) /
				Math.PI +
			90;
		ctx.beginPath();
		ctx.lineWidth = 3;
		...
		drawRotatedArrow(image, canvas.width / 2, canvas.height / 2, angle);
		ctx.globalAlpha = 1;
	} else {
		hero.trackerMaxFadeReached = 0;
...
function showMissionInPlay(hero, missionArray) {
	...
	for (h = 0; h < missionArray[hero.currentMission].length; h++) {
		if (missionArray[hero.currentMission][h].type === 'get') {
			document.getElementById('missionProgressContainer').innerHTML +=
				"<li><span id='missionContentSpan" +
				h +
				"'  class='missionContentSpan'>" +
				missionArray[hero.currentMission][h].statement +
				"<span class='amountDisplay'> " +
				missionArray[hero.currentMission][h].gathered +
				'/' +
				missionArray[hero.currentMission][h].amount +
				'</span></span></li>';
...
function showMissionTrackerCooldown(hero, timeControler) {
	if (hero.trackerActivated === 1) {
		...
		if (((hero.trackerCountdown + 15000 - timeControler.getTime()) / 1000).toFixed(1) <= 0) {
			...
			document.getElementById('gameAbilityOne').innerHTML = (
				(hero.trackerCountdown + 15000 - timeControler.getTime()) /
				1000
			).toFixed(1);
...
function recountGranade(gameArrays) {
	...
	document.getElementById('utilitiesGranade').innerHTML =
		'CTRL <br />' + "<span class='smallTextForUtilities'>" + gameArrays.granadeArray.length + ' Granade</span>';
}
...
function recountAntidote(hero) {
	...
	document.getElementById('utilitiesAntidote').innerHTML =
		'7 <br />' + "<span class='smallTextForUtilities'>" + hero.antidote + ' Antidote</span>';
}

function gameWonShowStats(
	archivedBulletArray,
	...
	document.getElementById('gameWonButton').onclick = function () {
		showStatistics(archivedBulletArray, archivedMonsterArray, numberOfDrops), cleanScreenForStatistics();
		setEndtime(gameVariables);
...
function reset(
	...
	for (y = 0; y < drops.length; y++) {
		...
		if (drops[y].itemType === 'schematic') {
			schematics[drops[y].imgName] = {
				obtained: 0,
				name: drops[y].name,
				constructor: drops[y].imgName,
				materials: drops[y].materials,
				description: drops[y].description,
			};
		}
	}
