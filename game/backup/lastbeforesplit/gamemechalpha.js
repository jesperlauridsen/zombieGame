...
var granadeArray = [];
var objectArray = [];
var schematicsyArray = [];
var objectImageArray = [];
var drops = [	
	{name:"Handgun rounds",imgName:"handgunRounds",amount:"7",offset:"15",itemType:"ammo"},
	{name:"Shotgun shells",imgName:"shotgunShells",amount:"8",offset:"16",itemType:"ammo"},
	{name:"Machinegun magazine",imgName:"machinegunMagazine",amount:"30",offset:"27",itemType:"ammo"},
	{name:"Flamethrower shells",imgName:"flamethrowerShells",amount:"50",offset:"27",itemType:"ammo"},
/*	{name:"Schematic: Rocketbooster boots",imgName:"schematicBoots",amount:"1",offset:"40",itemType:"schematic"},
	{name:"Schematic: Lazerscope",imgName:"schematicScope",amount:"1",offset:"30",itemType:"schematic"},
	{name:"Schematic: Radio",imgName:"schematicRadio",amount:"1",offset:"19",itemType:"schematic"},
	{name:"Schematic: Thermo-goggles",imgName:"schematicGoogles",amount:"1",offset:"33",itemType:"schematic"},
	{name:"Schematic: Pulse-Emitter",imgName:"schematicPulse",amount:"1",offset:"31",itemType:"schematic"},
	{name:"Schematic: Scavanger 101",imgName:"schematicTrash",amount:"1",offset:"31",itemType:"schematic"},	
	{name:"Desert Eagle",imgName:"pistol",amount:"1",offset:"10",itemType:"gun"},
	{name:"MAC-7",imgName:"shotgun",amount:"1",offset:"2",itemType:"gun"},
	{name:"AK-47",imgName:"machinegun",amount:"1",offset:"2",itemType:"gun"},
	{name:"M1A1 Flamethrower",imgName:"flamethrower",amount:"1",offset:"17",itemType:"gun"},*/
	{name:"Medkit",imgName:"medkit",amount:"1",offset:"3",itemType:"mat"},
	{name:"Antidote",imgName:"antidote",amount:"1",offset:"7",itemType:"mat"},
	{name:"Granade",imgName:"granade",amount:"1",offset:"5",itemType:"mat"},
	{name:"Broken scope",imgName:"brokenScope",amount:"1",offset:"10",itemType:"mat"},
	{name:"Lenses",imgName:"lenses",amount:"2",offset:"3",itemType:"mat"},
	{name:"Screws",imgName:"screws",amount:"5",offset:"3",itemType:"mat"},
	{name:"Metal scrap",imgName:"metalScrap",amount:"3",offset:"15",itemType:"mat"},
	{name:"Battery",imgName:"battery",amount:"1",offset:"5",itemType:"mat"},
	{name:"Electronic components",imgName:"electronicComponents",amount:"3",offset:"29",itemType:"mat"},
	{name:"Wires",imgName:"wires",amount:"4",offset:"4",itemType:"mat"},
	{name:"Steel case",imgName:"steelCase",amount:"1",offset:"8",itemType:"mat"},
	{name:"Small electronic screen",imgName:"smallElectronicScreen",amount:"1",offset:"33",itemType:"mat"},
	{name:"Broken antenna",imgName:"brokenAntenna",amount:"1",offset:"19",itemType:"mat"},
	{name:"Steel tube",imgName:"steelTube",amount:"1",offset:"8",itemType:"mat"},
	{name:"Glass shards",imgName:"glassShard",amount:"2",offset:"10",itemType:"mat"},
	{name:"Rocket fuel",imgName:"rocketFuel",amount:"1",offset:"8",itemType:"mat"},
	{name:"Speaker unit",imgName:"speakerUnit",amount:"1",offset:"10",itemType:"mat"},
	{name:"Leather straps",imgName:"leatherStraps",amount:"2",offset:"8",itemType:"mat"},
	{name:"Gaffa tape",imgName:"gaffaTape",amount:"1",offset:"8",itemType:"mat"},
	{name:"Handful of transistors",imgName:"transistors",amount:"10",offset:"32",itemType:"mat"}];
var inventory = {};
var thrownGranadeArray = [];
var archivedObjectArray = [];
var archivedBulletArray = [];
var archivedMonsterArray = [];
var archivedGranadeArray = [];
var missionType = "Flashlight";
var background = {};
var tileArray = {};
var keysDown = {};
var keyPressed = {};
...
addEventListener("keyup", function (e) {delete keysDown[e.keyCode]; if(hero.gun == 'pistol' || hero.gun == 'shotgun' || hero.gun == 'machete') {hasFired = 0;} else {isPressed = 0;}}, false);

// Reset the game when the player dies.
var reset = function () {
	for(y=4;y<drops.length;y++) {
	...
	hero.gunshots = 75;
	hero.shotgunshots = 40;
	hero.machinegunshots = 250;
	hero.flameshells = 500;
	backgroundArray = [];
...
var update = function (modifier) {
	...
	if (38 in keysDown) { // Player holding up - moving forward
		...
		for(o=0;o<objectArray.length;o++) {
			newPositionForward(objectArray[o]);
		}
	...
	if (40 in keysDown) { // Player holding down -  moving backwards
		...
		for(o=0;o<objectArray.length;o++) {
			newPositionBackward(objectArray[o]);
		}
		for(n=0;n<thrownGranadeArray.length;n++) {
	...
	for(i=0;i<bulletArray.length;i++) {
		...
		//if(bulletArray[i].bulletX <= (monsterArray[l].x + 30) && monsterArray[l].x <= (bulletArray[i].bulletX) && bulletArray[i].bulletY <= (monsterArray[l].y + 30) && monsterArray[l].y <= (bulletArray[i].bulletY)) {
				if(monsterArray[l].health-bulletArray[i].bulletDamage > 1) {
					...
					if(monsterArray[l].state != "attacking" || monsterArray[l].state != "startled" || monsterArray[l].state != "insane") {
						if(monsterArray[l].category == 1) {
							monsterArray[l].startledTime = timeControler.getTime();
							monsterArray[l].state = "startled";
						}
						else {
							monsterArray[l].startledTime = timeControler.getTime();
							monsterArray[l].state = "insane";
						}
					}
					else {
					}
	...
	// Hero touching any objects?
	for(v=0;v<objectArray.length;v++) {
		if(hero.x <= (objectArray[v].x + 18) && objectArray[v].x <= (hero.x + 18) && hero.y <= (objectArray[v].y + 18) && objectArray[v].y <= (hero.y + 18)) {
			numberOfDrops = numberOfDrops + 1;
			if(objectArray[v].name == "Handgun rounds") {
				hero.gunshots = hero.gunshots + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Shotgun shells") {
				hero.shotgunshots = hero.shotgunshots + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Machinegun magazine") {
				hero.machinegunshots = hero.machinegunshots + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Flamethrower shells") {
				hero.flameshells = hero.flameshells + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Medkit") {
				hero.medkit = hero.medkit + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Granade") {
				granadeToPlayer(200,75);
				objectArray.splice(v,1);
			}
			else if(objectArray[v].name == "Antidote") {
				hero.antidote = hero.antidote + parseInt(objectArray[v].amount);
				objectArray.splice(v,1);
			}
			else {
			//console.log(inventory[objectArray[v].imageSource].constructor + " | amount: " + inventory[objectArray[v].imageSource].amount);
			inventory[objectArray[v].imageSource].amount = parseInt(inventory[objectArray[v].imageSource].amount) + parseInt(objectArray[v].amount);
			objectArray[v].pickUpTime = timeControler.getTime();
			archivedObjectArray.push(objectArray[v]);
			objectArray.splice(v,1);
...
var render = function () {
	...
	for(i=0;i<objectArray.length;i++) {
		...
		ctx.font = "10px Lato";
		ctx.fillText(objectArray[i].name,objectArray[i].x-objectArray[i].offset,objectArray[i].y-3);
		ctx.drawImage(objectImageArray[i], objectArray[i].x, objectArray[i].y,30,30);
	}
	...
	if (monster1Ready && monster2Ready & monster3Ready & monster1IdleReady & monster2IdleReady & monster3IdleReady) {
		for(j=0;j<monsterArray.length;j++) {
			if(monsterArray[j].skin == 1) {
				if(monsterArray[j].state == "idle") {
					drawRotatedMonster(monsterImageIdle1, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
				else {
					drawRotatedMonster(monsterImage1, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
			}
			else if(monsterArray[j].skin == 2) {
				if(monsterArray[j].state == "idle") {
					drawRotatedMonster(monsterImageIdle2, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
				else {
					drawRotatedMonster(monsterImage2, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
			}
			else if(monsterArray[j].skin == 3) {
				if(monsterArray[j].state == "idle") {
					drawRotatedMonster(monsterImageIdle3, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
				else {
					drawRotatedMonster(monsterImage3, monsterArray[j].x, monsterArray[j].y, monsterArray[j].angle-180,30,30,monsterArray[j].state);
				}
			}
			else {
			}
	...
	if (lampReady && missionType == "Poles") {
	...
	else if(flashReady && missionType == "Flashlight" && hero.flashlight == "on") {
		drawRotatedFlashlight(flashImage, hero.x, hero.y, hero.angle);
	}
	else if(flashReady && missionType == "Flashlight" && hero.flashlight == "off") {
...
function monsterState(monster) {
	...
	if(monster.state == "idle") {
		...
		if(monster.setOffAngle == 1) {
			monster.setOffAngle = timeControler.getTime();
		}
		if(timeControler.getTime() < monster.startledTime + 6050) {
			if (hero.x <= (monster.x + 18) && monster.x <= (hero.x + 18) && hero.y <= (monster.y + 18) && monster.y <= (hero.y + 18)) {
			}
		else {
			if(timeControler.getTime() > monster.setOffAngle + 300 || monster.offAngle == "0") {
				monster.setOffAngle = timeControler.getTime();
...
function generateEnvironment(coreX, coreY) {
	arrayOfNewAdditions = [];
...
function monsterDrop(monster) {
	for(y=0;y<monster.drop;y++) {
		...
		var dropObject = {
			...
			dropTime:timeControler.getTime(),
			pickUpTime:0
		};
...
function newPickUpsDisplayed() {
	if(newPickUps > 0) {
	document.getElementById("pickUp").className = "pickedUp";
	document.getElementById("pickUp").innerHTML = newPickUps + "!";
	}
	else {
...
function inventoryReload() {
	var inventoryColor = "";
	if(document.getElementById("inventoryBackpack").className == "openInventory" && newPickUps > 0 || document.getElementById("inventoryBackpack").innerHTML == "") {
		document.getElementById("inventoryBackpack").innerHTML = "<div class='inventoryInside'><div class='exitInventory' onclick='showInventory()'>X</div><h4>Inventory</h4><break>";
		for(i=0;i<Object.keys(inventory).length;i++) {
			...
			document.getElementById("inventoryBackpack").innerHTML = document.getElementById("inventoryBackpack").innerHTML + "<span class='inventoryNameDisplay " + inventoryColor + "'> " +   inventory[Object.keys(inventory)[i]].name + ": </span> <span class='inventoryCountDisplay " + inventoryColor + "'>" + inventory[Object.keys(inventory)[i]].amount + "</span><break>";
		}
		document.getElementById("inventoryBackpack").innerHTML = document.getElementById("inventoryBackpack").innerHTML + "</div>";
		newPickUps = 0;
		}
		else {
		}
...
function showStatistics() {
	...
	document.getElementById("overallStatistics").innerHTML = "<strong>Overall</strong><br />Attacks made: " + archivedBulletArray.length + 
								...
								"Monsters killed: " + archivedMonsterArray.length + "<br > Granade kills: " + granadeKills + "<br />" +
								"Drops: " + numberOfDrops + "<br />" +
								"Dropchange per mob: " + dropchance + "% <br />";													
