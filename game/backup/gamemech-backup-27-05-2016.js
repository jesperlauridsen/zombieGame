...
var update = function (modifier) {
	...
	for(i=0;i<bulletArray.length;i++) {
		...
		if(bulletArray[i].bulletX <= (monsterArray[l].x + 18) && monsterArray[l].x <= (bulletArray[i].bulletX) && bulletArray[i].bulletY <= (monsterArray[l].y + 18) && monsterArray[l].y <= (bulletArray[i].bulletY)) {
				if(monsterArray[l].health-hero.damage > 1) {
				monsterArray[l].health = monsterArray[l].health-hero.damage;
					//document.getElementById("testdiv").innerHTML = "Monster health: " + monsterArray[l].health;
				}
				else if(monsterArray[l].health-hero.damage <= 0) {
...
var render = function () {
	...
	//drawTileShadow(numberOfLampsOnScreen[0].tileStartPointX,numberOfLampsOnScreen[0].tileStartPointY,numberOfLampsOnScreen[0].x,numberOfLampsOnScreen[0].y);
	
	//document.getElementById("testdiv").innerHTML = gameDisplay.indexX + " | " + gameDisplay.indexY + " | " + (gameDisplay.x) + "," + (gameDisplay.y) + " - " + tileDisplay.x + ", " + tileDisplay.y + "<br />" + numberOfLampsOnScreen;
	...
	try {
	document.getElementById("testdivon").innerHTML = "HERO: x: " + hero.x.toFixed(2) + " | y: " + hero.y.toFixed(2) + " - STAR: x: " + monsterArray[0].x.toFixed(2) + " | y: " + monsterArray[0].y.toFixed(2) + coalition + "<br />" + "Shots fired: " + shotCounter + " " + shoot;
	}
	catch(err) {
	}
	//Lamp
...
function runningAnimation() {
	...
	//Walking forward or Running backward
		...
		document.getElementById("testdiv").innerHTML = "HEY!";
...
function granadeExplosion(granade) {
	//console.log(timeControler.getTime()+25 + " > " + granade.activationTime.getTime());
	//if(granade.blown == 1 && timeControler.getTime()+25 > granade.activationTime.getTime()) {
...
function monsterStateRevision(monster) {
	...
	if(hero.walking == '1') {
		if(Math.sqrt((monster.x-400)*(monster.x-400) + (monster.y-300)*(monster.y-300)) < 100 + ((Math.random() * 5) -0)) {
		...
		if(Math.sqrt((monster.x-400)*(monster.x-400) + (monster.y-300)*(monster.y-300)) < 150 + ((Math.random() * 5) -0)) {
			monster.state = "attacking";
		}
		else {
			monster.state = "idle";
