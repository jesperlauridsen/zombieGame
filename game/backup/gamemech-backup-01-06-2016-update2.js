...
addEventListener("keyup", function (e) {
	...
	if(hero.gun == 'pistol' || hero.gun == 'shotgun') {
	hasFired = 0;
	}
	else {
	}
...
var update = function (modifier) {
	...
	if(32 in keysDown && hasFired == '0'){ //Shot
...
var render = function () {
	...
	for(h=0;h<monsterArray.length;h++) {
		if(monsterArray[h].hit != 0) {
			monsterHitOnHero(monsterArray[h]);
		}
	}
};
...
function monsterHitOnHero(monster) {
	if(timeControler.getTime() < monster.hit + 150) {
		if(monster.monsterHitOpacity < 0.9 && monster.monsterHitPeaked == 0) {
		monster.monsterHitOpacity = monster.monsterHitOpacity + 0.2;
		ctx.globalAlpha = monster.monsterHitOpacity;
		ctx.drawImage(monsterHitImage,0,0,800,600);
		//ctx.globalAlpha = 1;
		}
		else {
		monster.monsterHitPeaked = 1;
		monster.monsterHitOpacity = monster.monsterHitOpacity - 0.2;
		ctx.globalAlpha = monster.monsterHitOpacity;
		ctx.drawImage(monsterHitImage,0,0,800,600);
		ctx.globalAlpha = 1;
...
function bulletTraceFade(bullet) {
	...
	ctx.fillStyle = 'rgba(255,' + blueFiler + ',0,' + opacityFiler + ')';
	}
	else if(hero.gun == "machinegun") {
