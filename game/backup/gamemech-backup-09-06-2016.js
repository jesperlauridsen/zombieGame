...
randomStartNumber = Math.round(Math.random() * 1 + 2);
	for(x=0;x<randomStartNumber;x++) {
			spawnMonster(0, -600,20,500,100,100,1);
	}
...
randomStartNumber = Math.round(Math.random() * 1 + 2);
	for(x=0;x<randomStartNumber;x++) {
			spawnMonster(-800,0,20,500,100,100,1);
	}
...
randomStartNumber = Math.round(Math.random() * 1 + 2);
	for(x=0;x<randomStartNumber;x++) {
			spawnMonster(800,-600,20,500,100,100,1);
	}
...
var update = function (modifier) {
	...
	if(32 in keysDown && hasFired == '0'){ //Shoot
			...
			else if(hero.gun == 'machete' && timeControler.getTime() > hero.macheteSwing+200) {
				delete keysDown[32];
				hero.macheteSwing = timeControler.getTime();
				shotCounter = shotCounter + 1;
				hasFired = 1;
				//console.log("machete swing!");
	...
	for(g=0;g<bulletArray.length;g++) {
		...
		if(bulletArray[g].firedFrom == 'flamethrower') {
			if(Math.sqrt(Math.pow((bulletArray[g].bulletX-hero.x),2) + Math.pow((bulletArray[g].bulletY-hero.y),2)) > 150) {
				...
				//console.log(bulletArray[g].angleCount);
			if(bulletArray[g].angleCount < 10) {
				bulletTrace(bulletArray[g]);
				 	//bullet.bulletX = Math.round(hero.x + Math.cos(bullet.bulletStartAngle * TO_RADIANS) * 15);
				 	//bullet.bulletY = Math.round(hero.y + Math.sin(bullet.bulletStartAngle * TO_RADIANS) * 15);
					ctx.beginPath();
					ctx.lineWidth = 3;
					ctx.strokeStyle = 'rgba(255,230,87,0.7)';
					ctx.arc(bulletArray[g].bulletX,bulletArray[g].bulletY,5,0,2*Math.PI);
...
function bulletTrace(bullet) {
	if(bullet.firedFrom == 'machete') {
		...
		bullet.bulletStartAngle = bullet.bulletStartAngle - 15;
...
function fixThisLampShit(coreX, coreY) {
	...
	if(realityStatus == true) {
		...
		var randomNumber = Math.round(Math.random() * 1 + 8);
...
function bulletFire(damageDealt) {
	...
	if(hero.gun == 'shotgun') {
		...
		bulletAngleFromHero = Math.round(hero.angle + 60);
		startX = Math.round(hero.x + Math.cos(bulletAngleFromHero * TO_RADIANS) * 25);
		startY = Math.round(hero.y + Math.sin(bulletAngleFromHero * TO_RADIANS) * 25);
	}
	else {
