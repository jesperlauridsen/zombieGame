...
function monsterState(monster, hero, gameVariables, missionArray, thrownGranadeArray, bulletArray) {
	...
	if (monster.state == 'idle') {
		...
		if (
			Math.sqrt(
				(monster.x - canvas.width / 2) * (monster.x - canvas.width / 2) +
					(monster.y - canvas.height / 2) * (monster.y - canvas.height / 2)
			) < 150
		) {
			...
			if (
				missionArray[12][0].shotTimer > gameVariables.timeControler.getTime() &&
				missionArray[12][0].shotState === 1
			) {
				if (missionArray[12][0].shotWave < 7) {
					if (
						missionArray[12][0].shotTimer - (1400 - 200 * missionArray[12][0].shotWave) <
						gameVariables.timeControler.getTime()
					) {
						missionArray[12][0].shotWave = missionArray[12][0].shotWave + 1;
						firingAngle =
							(Math.atan2(hero.y - 10 - monster.y, hero.x - 10 - monster.x) * 180) / Math.PI +
							75 +
							5 * missionArray[12][0].shotWave;
						bulletFire(25, monster, bulletArray, gameVariables, firingAngle);
					}
				}
			}
			if (
				missionArray[12][0].shotTimer + 5000 < gameVariables.timeControler.getTime() &&
				missionArray[12][0].shotState === 1
			) {
...
function monsterStateRevision(monster, hero) {
	...
	if (
		monster.state == 'startled' ||
		monster.state == 'insane' ||
		monster.state == 'desperate' ||
		monster.state == 'boss'
	) {
	} else {
		if (hero.walking == '1') {
