...
function selectWeapon(weapon, hero) {
	...
	if (hero[weapon] === 1) {
		...
		document.getElementById('weapon' + weapon[0].toUpperCase() + weapon.substring(1)).style.borderStyle =
			'none solid none solid';
		document.getElementById('weapon' + weapon[0].toUpperCase() + weapon.substring(1)).style.boxShadow =
			'0 8px 16px -6px black';
	} else {
	}
}
...
function showPulseTransmitterCooldown(hero, timeControler) {
	if (hero.pulseTransmitterFired === 1) {
		...
		if (
			((hero.pulseTransmitterCountdown + 30000 - timeControler.getTime()) / 1000).toFixed(1) <= 0 &&
			...
			document.getElementById('gameAbilitySix').innerHTML = (
				(hero.pulseTransmitterCountdown + 30000 - timeControler.getTime()) /
				1000
			).toFixed(1);
	...
	if (hero.pulseTransmitterFired === 1) {
		if (hero.pulseTransmitterFiredTime + 300 > timeControler.getTime()) {
			...
			for (h = 0; h < monsterArray.length; h++) {
				if (
					(monsterArray[h].x - hero.x) * (monsterArray[h].x - hero.x) +
						(monsterArray[h].y - hero.y) * (monsterArray[h].y - hero.y) <
					j * j
				) {
...
function thrownGranade(
	...
	if ((granade.thrown = 1 && timeControler.getTime() < granade.activationTime.getTime())) {
		...
		ctx.font = '10px Lato';
		ctx.fillText(
			((granade.activationTime.getTime() - timeControler.getTime() + 10) / 500).toFixed(1),
			granade.x - 8,
			granade.y - (granade.radius + 5)
		);
		for (x = 0; x < 50; x++) {
			ctx.beginPath();
		...
		if (timeControler.getTime() + 25 >= granade.activationTime.getTime() && granade.blown == 0) {
			...
			for (i = 0; i < monsterArray.length; i++) {
				...
				if (
					(monsterArray[i].x - granade.x) * (monsterArray[i].x - granade.x) +
						(monsterArray[i].y - granade.y) * (monsterArray[i].y - granade.y) <
					granade.radius * granade.radius
				) {
					//console.log("yes");
			...
			if (
				(hero.x - granade.x) * (hero.x - granade.x) + (hero.y - granade.y) * (hero.y - granade.y) <
				granade.radius * granade.radius
			) {
				...
				if (
					Math.round(
						granade.damage *
							((granade.radius -
								Math.round(
									Math.sqrt((hero.x - granade.x) * (hero.x - granade.x) + (hero.y - granade.y) * (hero.y - granade.y))
								)) /
								200) >
							hero.health
					)
...
function granadeExplosion(granade) {
	if (granade.explosionExpansion > 1.3 && granade.explosionExpansionTopped == 0) {
...
function granadeStatus(gameArrays, gameVariables) {
	if (gameArrays.granadeArray.length > 0) {
		if (
			...
			gameArrays.granadeArray[gameArrays.granadeArray.length - 1].activationTime.setTime(
				gameVariables.timeControler.getTime() + 500
			);
			...
			gameArrays.granadeArray[gameArrays.granadeArray.length - 1].activated === 1 &&
			gameArrays.granadeArray[gameArrays.granadeArray.length - 1].inbound === 0
		) {
			//Reset the granade
...
function throwGranade(granade, hero) {
	if (granade.activated == 1) {
		if (
			Math.sqrt(
				(canvas.width / 2 - granade.x) * (canvas.width / 2 - granade.x) +
					(canvas.height / 2 - granade.y) * (canvas.height / 2 - granade.y)
			) < 500
		) {
