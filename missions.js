function initiateMissions(
	...
	for (y = 0; y < Object.keys(environmentalPoints).length; y++) {
		...
		missionPointX = Math.floor(missionPointX / canvas.width);
		missionPointY = Math.floor(missionPointY / canvas.height);
	...
	for (y = 0; y <= 16; y++) {
		...
		var dropObject = {
			...
			name: questDropName,
			itemType: 'questItem',
			x: Math.round(Math.random() * canvas.width * 3 - 1) - canvas.width + missionArray[9][0].indexX * canvas.width, //Random
			y: Math.round(Math.random() * canvas.height * 3 - 1) - canvas.height + missionArray[9][0].indexY * canvas.height, //Random
...
function validateMission(
	...
	for (h = 0; h < missionArray[hero.currentMission].length; h++) {
		if (
			missionArray[hero.currentMission][h].type === 'get' &&
			missionArray[hero.currentMission][h].func === 'primary'
		) {
			...
			missionArray[hero.currentMission][h].type === 'find' &&
			missionArray[hero.currentMission][h].func === 'primary'
		) {
			if (hero.currentMission === 9) {
				if (
					(missionArray[hero.currentMission][h].indexX === gameDisplay.indexX &&
						missionArray[hero.currentMission][h].indexY === gameDisplay.indexY) ||
					(missionArray[hero.currentMission][h].indexX === gameDisplay.indexX - 1 &&
						missionArray[hero.currentMission][h].indexY === gameDisplay.indexY - 1) ||
					(missionArray[hero.currentMission][h].indexX === gameDisplay.indexX - 1 &&
						missionArray[hero.currentMission][h].indexY === gameDisplay.indexY) ||
					(missionArray[hero.currentMission][h].indexX === gameDisplay.indexX - 1 &&
						missionArray[hero.currentMission][h].indexY === gameDisplay.indexY + 1) ||
			...
			missionArray[hero.currentMission][h].type === 'kill' &&
			missionArray[hero.currentMission][h].func === 'primary'
		) {
			if (missionArray[hero.currentMission][h].gathered >= missionArray[hero.currentMission][h].amount) {
				missionArray[hero.currentMission][h].completionTime = timeControler.getTime();
			...
			missionArray[hero.currentMission][h].type === 'interact' &&
			missionArray[hero.currentMission][h].func === 'primary'
		) {
			...
			missionArray[hero.currentMission][h].type === 'win' &&
			missionArray[hero.currentMission][h].func === 'primary'
		) {
...
function presentMission(missionArray, hero, timeControler) {
	if (hero.currentMission === 0) {
		document.getElementById('missionInteractionContainer').getElementsByTagName('p')[0].innerHTML =
			missionArray[hero.currentMission][0].message;
	} else {
		//console.log(missionArray[hero.currentMission-1][0].completion);
		document.getElementById('missionInteractionContainer').getElementsByTagName('p')[0].innerHTML =
			missionArray[hero.currentMission - 1][0].completion +
			'<br /><br />' +
			missionArray[hero.currentMission][0].message;
	}
	document.getElementById('missionInteractionContainer').className = 'progressActive';
...
function ambushMissionSpawn(hero, objectArray, timeControler, missionArray, monsterArray, granadeArray, rocketArray) {
	if (hero.currentMission === 6) {
		...
		if (
			missionArray[5][0].completionTime + missionArray[6][0].wave * 7000 < timeControler.getTime() &&
			missionArray[6][0].wave < 7
		) {
			//new wave
		...
		if (
			missionArray[5][0].completionTime + missionArray[6][1].wave * 12000 < timeControler.getTime() &&
			missionArray[6][1].wave < 4
		) {
			//new wave
		...
		if (
			missionArray[5][0].completionTime + missionArray[6][2].wave * 17000 < timeControler.getTime() &&
			missionArray[6][2].wave < 4
		) {
			//new wave
	...
	if (hero.currentMission === 8) {
		if (
			missionArray[7][0].completionTime + missionArray[8][0].wave * 6000 < timeControler.getTime() &&
			missionArray[8][0].wave < 13
		) {
			//new wave
			//console.log("new wave of zombies to repel! (wave " + missionArray[8][0].wave + ")");
			for (x = 0; x < 2; x++) {
				var randomAngle = Math.round(Math.random() * 360 + 0);
				var xPoint = Math.round(canvas.width / 2 + Math.cos(randomAngle) * Math.round(Math.random() * 1000 + 800));
				var yPoint = Math.round(canvas.height / 2 + Math.sin(randomAngle) * Math.round(Math.random() * 1000 + 800));
				// spawnMonster(xStart, yStart,damage,damageInterval,health,fullHealth,category,monsterArray,state) {
	...
	if (hero.currentMission === 12) {
		if (
			missionArray[11][0].completionTime + (missionArray[12][0].wave * 5000 - 500 * missionArray[12][0].wave) <
			timeControler.getTime()
		) {
			//new wave
			if (missionArray[12][0].wave === 3 && missionArray[12][0].bossSpawned === 0) {
				var randomAngle = Math.round(Math.random() * 360 + 0);
