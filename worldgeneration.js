...
function generateEnvironment(
	...
	arrayOfNewAdditions.push({
		x: coreX,
		y: coreY - 1,
		startPointX: 0 + (canvas.width / 2 - tileDisplay.x),
		startPointY: -(canvas.height * 1.5) + (canvas.height - tileDisplay.y),
	});
	...
	arrayOfNewAdditions.push({
		...
		y: coreY + 1,
		startPointX: 0 + (canvas.width / 2 - tileDisplay.x),
		startPointY: canvas.height * 1.5 - tileDisplay.y,
	});
	...
	arrayOfNewAdditions.push({
		x: coreX - 1,
		y: coreY,
		startPointX: -(canvas.width * 1.5) + (canvas.width - tileDisplay.x),
		startPointY: 0 + (canvas.height / 2 - tileDisplay.y),
	});
	...
	arrayOfNewAdditions.push({
		x: coreX + 1,
		y: coreY,
		startPointX: canvas.width * 1.5 - tileDisplay.x,
		startPointY: 0 + (canvas.height / 2 - tileDisplay.y),
	});
	...
	arrayOfNewAdditions.push({
		x: coreX + 1,
		y: coreY - 1,
		startPointX: canvas.width + (canvas.width / 2 - tileDisplay.x),
		startPointY: -canvas.height + (canvas.height / 2 - tileDisplay.y),
	});
	...
	for (g = 0; g < arrayOfNewAdditions.length; g++) {
		for (l = 0; l < numberOfLampsOnScreen.length; l++) {
			if (
				arrayOfNewAdditions[g].x === numberOfLampsOnScreen[l].indexX &&
				arrayOfNewAdditions[g].y === numberOfLampsOnScreen[l].indexY
			) {
				realityStatus = false;
		...
		if (realityStatus === true) {
			//Tilføj lampe
			addLampToScreenArrayVersion2(
				arrayOfNewAdditions[g].x,
				arrayOfNewAdditions[g].y,
				arrayOfNewAdditions[g].startPointX,
				arrayOfNewAdditions[g].startPointY,
				300,
				numberOfLampsOnScreen
			);
			spawnBoxInTile(
			...
			if (
				arrayOfNewAdditions[g].x == missionArray[0][0].indexX &&
				arrayOfNewAdditions[g].y == missionArray[0][0].indexY
			) {
				//Throw down basecamp!
				...
				arrayOfNewAdditions[g].x == missionArray[1][0].indexX &&
				arrayOfNewAdditions[g].y == missionArray[1][0].indexY
			) {
				//Throw down farm!
				...
				arrayOfNewAdditions[g].x == missionArray[5][0].indexX &&
				arrayOfNewAdditions[g].y == missionArray[5][0].indexY
			) {
				...
				(arrayOfNewAdditions[g].x == missionArray[9][0].indexX &&
					arrayOfNewAdditions[g].y == missionArray[9][0].indexY) ||
				(arrayOfNewAdditions[g].x == missionArray[9][0].indexX - 1 &&
					arrayOfNewAdditions[g].y == missionArray[9][0].indexY - 1) ||
				(arrayOfNewAdditions[g].x == missionArray[9][0].indexX - 1 &&
					arrayOfNewAdditions[g].y == missionArray[9][0].indexY) ||
				(arrayOfNewAdditions[g].x == missionArray[9][0].indexX - 1 &&
					arrayOfNewAdditions[g].y == missionArray[9][0].indexY + 1) ||
				...
				arrayOfNewAdditions[g].x == missionArray[11][0].indexX &&
				arrayOfNewAdditions[g].y == missionArray[11][0].indexY
			) {
...
function spawnOverlayEnvironment(argument, treeAndHouseArray, treeImagesLoaded, indexX, indexY, x, y) {
	if (x === undefined) {
		x = canvas.width * indexX;
	}
	if (y === undefined) {
		y = canvas.width * indexY;
	}
	if (argument === 'tree') {
...
function setSpecificEnvironment(
	...
	if (name === 'forest') {
		...
		treeObject.width = canvas.width; //treeSize;
		treeObject.height = canvas.height; // treeSize;
...
function drawTileShadow(tileStartPointX, tileStartPointY, lightX, lightY, radius) {
	...
	ctx.fillRect(
		lightX + imageLength,
		tileStartPointY,
		canvasLength - (lightX - tileStartPointX + imageLength),
		lightY - tileStartPointY
	);
	...
	ctx.fillRect(
		tileStartPointX,
		lightY + imageLength,
		lightX - tileStartPointX,
		canvasHeight - (lightY - tileStartPointY + imageLength)
	);
